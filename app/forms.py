...
from config.default import ALLOWED_EXTENSIONS
...
class SignUpForm(FlaskForm):
    ...
    profile_image = FileField('프로필 사진', validators=[FileAllowed(allowed, '프로필 사진은 이미지 파일만 업로드할 수 있습니다.')])
...
class ProfileModifyForm(FlaskForm):
    username = StringField('아이디', validators=[DataRequired('아이디는 필수입력 항목입니다.'), Length(min=3, max=25, message='아이디는 3~25자 이내여야 합니다'), Regexp('^[A-Za-z][A-Za-z0-9]*$', message='아아디는 영문 및 숫자만 사용할 수 있습니다.')])
    nickname = StringField('닉네임', validators=[DataRequired('닉네임은 필수입력 항목입니다.'), Length(min=2, max=25, message='닉네임은 2~25자 이내여야 합니다')])
    password = PasswordField('비밀번호', validators=[EqualTo('password_validate', '비밀번호가 일치하지 않습니다')])
    password_validate = PasswordField('비밀번호 확인')
    email = EmailField('이메일', validators=[DataRequired('이메일은 필수입력 항목입니다.'), Email('이메일 형식이 올바르지 않습니다.')])
    profile_image = FileField('프로필 사진', validators=[FileAllowed(allowed, '프로필 사진은 이미지 파일만 업로드할 수 있습니다.')])
    about_me = StringField('자기소개')
...
class MageForm(FlaskForm):
    ...
    image = FileField('균열 마법사 이미지', validators=[FileAllowed(allowed, '균열 마법사 이미지는 이미지 파일만 업로드할 수 있습니다.')])
    board_image = FileField('균열 마법사 보드 이미지', validators=[FileAllowed(allowed, '균열 마법사 보드 이미지는 이미지 파일만 업로드할 수 있습니다.')])
    back_board_image = FileField('균열 마법사 보드 뒷면 이미지', validators=[FileAllowed(allowed, '균열 마법사 보드 뒷면 이미지는 이미지 파일만 업로드할 수 있습니다.')])
...
class CardForm(FlaskForm):
    ...
    image = FileField('카드 이미지', validators=[FileAllowed(allowed, '카드 이미지는 이미지 파일만 업로드할 수 있습니다.')])
...
class NemesisForm(FlaskForm):
    ...
    image = FileField('네메시스 이미지', validators=[FileAllowed(allowed, '네메시스 이미지는 이미지 파일만 업로드할 수 있습니다.')])
    board_image = FileField('네메시스 보드 이미지', validators=[FileAllowed(allowed, '네메시스 보드 이미지는 이미지 파일만 업로드할 수 있습니다.')])
    back_board_image = FileField('네메시스 보드 뒷면 이미지', validators=[FileAllowed(allowed, '네메시스 보드 뒷면 이미지는 이미지 파일만 업로드할 수 있습니다.')])
